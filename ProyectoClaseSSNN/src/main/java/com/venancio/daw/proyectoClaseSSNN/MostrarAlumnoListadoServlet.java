...
public class MostrarAlumnoListadoServlet extends GenericServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int codigo = (int) request.getAttribute("idAlumno");
        List<AlumnosNotas> allNotasAlum = aN.mostrarNotaAlumno(codigo);
        
        ctx.setVariable("listaNotAlum", allNotasAlum);
        
        TemplateEngine engine = configThymeleaf.getTemplateEngine();
        
        engine.process("mostrarAlumnoListado", ctx, response.getWriter());
